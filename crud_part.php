<?php
   ...
   $select = "SELECT * FROM trainee ORDER BY id DESC";
      ...
      <div class="container">
         <table id="participants_table" class="display" width="100%" cellspacing="">
            ...
            <tbody>
               ...
               <tr>
                  <td><?php echo $row['title']. ' '. $row['fname'] . ' ' . $row['mname'] . ' ' . $row['lname'].' '. $row['suffix']; ?></td>
                  <td><?php echo $row['fac_name']; ?></td>
                  <td><?php echo $row['cert_no']; ?></td>
                  <td><?php echo $row['type_of_cert']; ?></td>
                  <td><?php echo $row['hrs']; ?></td>
                  <td>
                     <!-- show topics -->
                     <button onclick="call(<?php
                        echo $row['id']; ?>)" type="button" name="view_topic" class="view_topic btn btn-info btn-sm" data-toggle="modal" data-target="#topic_modal_part" data-toggle="tooltip" data-placement="top" title="View Topics"><i class="far fa-file" ></i></button>
                  </td>
                  <td>
                     ...
                     <a href="crud_process.php?del=<?php
                        echo $row['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are You sure you want to Delete this Record?')"data-toggle="tooltip" data-placement="top" title="Delete"><i class="far fa-trash-alt" ></i></a>
                     <!-- award button -->
